...
const layoutStart = content.indexOf('function Layout({ children, user, isAdmin: isUserAdmin }');
if (layoutStart !== -1) {
    const returnStart = content.indexOf('return (', layoutStart);
    const layoutEnd = content.indexOf('function CandidatePortalGuard', returnStart);
    if (returnStart !== -1 && layoutEnd !== -1) {
        const endBrace = content.lastIndexOf('}', layoutEnd);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100 flex flex-col">
      ...
      <footer className="bg-white border-t border-slate-200 py-12 mt-auto">
        ...
        content = content.substring(0, returnStart) + newReturn + '\n' + content.substring(endBrace + 1);
        fs.writeFileSync(file, content);
        console.log('Layout updated successfully');
    } else {
        console.log('Could not find Layout return block boundaries');
    }
} else {
    console.log('Could not find Layout component');
}
