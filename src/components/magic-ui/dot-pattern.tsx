...
export function DotPattern({
  width = 16, height = 16, x = 0, y = 0, cx = 1, cy = 1, cr = 1, className, glow = false, ...props
}: DotPatternProps) {
  const id = useId()
  ...
  const dots = Array.from({ length: Math.ceil(dimensions.width / width) * Math.ceil(dimensions.height / height) }, (_, i) => {
    ...
    return { x: col * width + cx + x, y: row * height + cy + y, delay: Math.random() * 5, duration: Math.random() * 3 + 2 }
  })
