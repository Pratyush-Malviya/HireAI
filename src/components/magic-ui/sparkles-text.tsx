...
export const SparklesText: React.FC<SparklesTextProps> = ({
  children, colors = { first: "#9E7AFF", second: "#FE8BBB" }, className, sparklesCount = 10, ...props
}) => {
  ...
  useEffect(() => {
    const generateStar = (): Sparkle => {
      ...
      return { id: `${starX}-${starY}-${Date.now()}`, x: starX, y: starY, color, delay, scale, lifespan }
    }
    const initializeStars = () => setSparkles(Array.from({ length: sparklesCount }, generateStar))
    const updateStars = () => setSparkles((current) => current.map((star) => star.lifespan <= 0 ? generateStar() : { ...star, lifespan: star.lifespan - 0.1 }))
    initializeStars()
