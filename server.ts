...
app.post("/api/candidate/send-invite", async (req, res) => {
  ...
  if (interviewLink && typeof interviewLink === "string") {
    const isSafe = interviewLink.startsWith('/') || /^(https?:\/\/)/.test(interviewLink);
    if (isSafe) {
...
app.post("/api/admin/test-smtp", async (req, res) => {
  ...
  try {
    ...
    // 2. Send test email
    const fromName = smtpFromName || "HireNow Gateway Test";
    ...
    await transporter.sendMail({
      from: fromString,
      to: testRecipient,
      subject: "HireNow SMTP Mail Server Configuration - Test Success",
      html: `
        <!DOCTYPE html>
        <html>
          <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>SMTP Verification Success</title>
            <style>
              body {
                margin: 0; padding: 0; width: 100% !important; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; background-color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
              }
              @media only screen and (max-width: 600px) {
                ...
                .meta-table td:first-child {
                  font-weight: bold !important; color: #4f46e5 !important; margin-top: 8px !important;
                }
              }
            </style>
          </head>
          <body>
            <div class="email-container" style="max-width: 600px; margin: 0 auto; padding: 40px 20px; color: #334155; line-height: 1.6;">
              ...
              <div class="email-card" style="background-color: #ffffff; border-radius: 20px; border: 1px solid #e2e8f0; padding: 36px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05); border-top: 4px solid #10b981;">
                <div style="background-color: #d1fae5; border: 1px solid #a7f3d0; border-radius: 8px; padding: 10px 16px; margin-bottom: 20px; color: #065f46; font-weight: 800; font-size: 13px; display: inline-block; text-transform: uppercase; letter-spacing: 0.05em;">
                  ✓ Live SMTP Connection Succeeded
                </div>
                <h2 style="color: #0f172a; margin-top: 0; font-size: 18px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 15px;">SMTP Server Test Successful</h2>
                <p style="margin: 0 0 12px 0; font-size: 14px; color: #475569; font-weight: 500;">Congratulations! Your workspace has successfully linked its custom SMTP mail delivery server to HireNow.</p>
                <p style="margin: 0 0 24px 0; font-size: 14px; color: #475569; font-weight: 500;">All recruitment invitations, scorecard dispatches, and notifications for candidates will now be dispatched from your company domain.</p>
                
                <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; margin-bottom: 20px; font-size: 13px;">
                  <table class="meta-table" style="width: 100%; border-collapse: collapse;">
                    <tr><td style="padding: 6px 0; color: #64748b; font-weight: bold; width: 120px;">Host:</td><td style="padding: 6px 0; color: #0f172a; font-family: monospace; font-weight: 600;">${smtpHost}</td></tr>
                    <tr><td style="padding: 6px 0; color: #64748b; font-weight: bold;">Port:</td><td style="padding: 6px 0; color: #0f172a; font-family: monospace; font-weight: 600;">${smtpPort}</td></tr>
                    <tr><td style="padding: 6px 0; color: #64748b; font-weight: bold;">Secure:</td><td style="padding: 6px 0; color: #0f172a; font-family: monospace; font-weight: 600;">${smtpSecure ? 'SSL/TLS (secure)' : 'Non-secure (STARTTLS)'}</td></tr>
                    <tr><td style="padding: 6px 0; color: #64748b; font-weight: bold;">Sender Name:</td><td style="padding: 6px 0; color: #0f172a; font-family: monospace; font-weight: 600;">${fromName}</td></tr>
                    <tr><td style="padding: 6px 0; color: #64748b; font-weight: bold;">Sender Email:</td><td style="padding: 6px 0; color: #0f172a; font-family: monospace; font-weight: 600;">${fromEmail}</td></tr>
                  </table>
                </div>

                <div style="border-top: 1px solid #f1f5f9; padding-top: 20px; margin-top: 28px; font-size: 11px; color: #94a3b8; text-align: center; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">
                  This test email was dispatched automatically when configuring custom mail settings on HireNow.
                </div>
              </div>
            </div>
          </body>
