...
var COMPOSIO_API_KEY = process.env.COMPOSIO_API_KEY;
var composio = COMPOSIO_API_KEY ? new Composio({ apiKey: COMPOSIO_API_KEY }) : null;
var app = express();
var PORT = 3e3;
app.set("trust proxy", 1);
app.use(helmet({
  contentSecurityPolicy: false,
  // Prevent asset-blocking or sandbox failures
  frameguard: false,
  // Allow rendering inside AI Studio container frames
  crossOriginOpenerPolicy: false,
  // Allow OAuth Google popup scopes to send parent postMessages
...
var generalLimit = rateLimit({
  ...
  // 15 minutes
  max: 350,
  // max 350 requests per client per window
  message: { error: "General request quota exceeded. Please try again shortly." },
  standardHeaders: true,
  legacyHeaders: false,
  validate: { forwardedHeader: false }
});
var aiQuotaLimit = rateLimit({
  windowMs: 60 * 1e3,
  // 1 minute
  max: 15,
  // max 15 heavy AI audit prompts per client per minute
  message: { error: "AI processing capacity reached. Please hold for 1 minute before your next talent audit." },
  standardHeaders: true,
  legacyHeaders: false,
  validate: { forwardedHeader: false }
});
var deliveryLimiter = rateLimit({
  windowMs: 10 * 60 * 1e3,
  // 10 minutes
  max: 8,
  // max 8 external candidate invite notifications per client to block automated mail spam
  message: { error: "Notification dispatch rate exceeded. Please try again in 10 minutes." },
  standardHeaders: true,
  legacyHeaders: false,
  validate: { forwardedHeader: false }
});
var nvidiaQuotaLimit = rateLimit({
  windowMs: 60 * 1e3,
  // 1 minute
  max: 20,
  // max 20 NVIDIA NIM requests per client per minute
  message: { error: "NVIDIA AI capacity reached. Please hold for 1 minute." },
  standardHeaders: true,
  legacyHeaders: false,
  validate: { forwardedHeader: false }
});
var safeSanitize = (unsafe) => {
  ...
  if (useComposio && composio && userId) {
    try {
      const response = await composio.tools.execute(userId, "GOOGLECALENDAR_CREATE_EVENT", {
        ...
        event_duration_minutes: Math.round((new Date(endTime).getTime() - new Date(startTime).getTime()) / (60 * 1e3)),
...
app.post("/api/candidate/send-invite", async (req, res) => {
  ...
  console.log("\u{1F50E} send-invite request body:", req.body);
  if (useComposio && composio && userId) {
    try {
      const cleanName2 = safeSanitize(candidateName || "Candidate").substring(0, 100);
      const cleanTitle2 = safeSanitize(jobTitle || "Applied Position").substring(0, 150);
      const subject2 = subjectOverride || `Interview Invitation: ${cleanTitle2} with HireNow`;
      const body = emailBody || `Hi ${cleanName2},

You are invited to complete an interview for the position of ${cleanTitle2}.
...
Please join the interview room here: ${interviewLink}`;
  ...
  if (tokensRaw) {
    try {
      ...
      const oauth2Client = getOAuthClient();
      oauth2Client.setCredentials(tokens);
      const makeBody = (to, subjectStr, htmlContent) => {
  ...
  if (!emailSent && customSmtp && (customSmtp.smtpUser || customSmtp.user) && (customSmtp.smtpPass || customSmtp.pass)) {
    try {
      ...
      const secure = (customSmtp.smtpSecure !== void 0 ? customSmtp.smtpSecure : customSmtp.secure) !== false;
...
async function generateContentWithRetry(params, maxRetries = 3, initialDelay = 1e3) {
  const modelsToTry = [
    params.model,
    "gemini-flash-latest",
    "gemini-3.5-flash",
          ...
          if (attempt < maxRetries) {
            if (message?.includes("PerDay") && !message?.includes("retry in")) {
              ...
              if (retryMatch) {
                ...
                delay = Math.max(delay, (waitTime + 0.5) * 1e3);
              }
              const jitter = Math.random() * 500;
              delay = Math.min(delay + jitter, 8e3);
...
function parseJobFallback(text) {
...
function screenCandidateFallback(resumeText, jobRequirements) {
  ...
  const mustHaves = Array.isArray(jobRequirements?.must_have_skills) ? jobRequirements.must_have_skills : ["React", "TypeScript", "Node.js"];
  const niceHaves = Array.isArray(jobRequirements?.nice_to_have_skills) ? jobRequirements.nice_to_have_skills : ["CSS", "Docker"];
  const confirmed = [];
  const absent = [];
  const inferred = [];
...
function researchCandidateFallback(candidateName, role, company, details) {
...
function chatFallback(candidateName, role, company, jd, resume, history) {
...
function summarizeFallback(history) {
...
var CANDIDATE_SCREENING_SCHEMA = {
  type: Type.OBJECT,
    ...
    if (result.scorecard) {
      ...
      if (result.scorecard.dimensions) {
        ...
        if (dims.redFlags && Array.isArray(dims.redFlags.flags)) {
          dims.redFlags.flags.forEach((flag) => {
...
app.post("/api/ai/research-candidate", async (req, res) => {
  ...
  try {
    ...
    const finalResult = {
      ...
      identity_confidence: jsonResult.identity_confidence !== void 0 ? jsonResult.identity_confidence : 85,
      technical_score: jsonResult.technical_score !== void 0 ? jsonResult.technical_score : 80,
      leadership_score: jsonResult.leadership_score !== void 0 ? jsonResult.leadership_score : 75,
      communication_score: jsonResult.communication_score !== void 0 ? jsonResult.communication_score : 80,
      reputation_score: jsonResult.reputation_score !== void 0 ? jsonResult.reputation_score : 70,
      risk_score: jsonResult.risk_score !== void 0 ? jsonResult.risk_score : 10,
      ...
      engineering_depth_score: jsonResult.engineering_depth_score !== void 0 ? jsonResult.engineering_depth_score : 80,
      problem_solving_score: jsonResult.problem_solving_score !== void 0 ? jsonResult.problem_solving_score : 80,
      stability_score: jsonResult.stability_score !== void 0 ? jsonResult.stability_score : 85,
      growth_trajectory: jsonResult.growth_trajectory || "Progressive performance across employment positions.",
      industry_visibility_score: jsonResult.industry_visibility_score !== void 0 ? jsonResult.industry_visibility_score : 60,
    ...
    const sources = chunks ? chunks.filter((c) => c.web).map((c) => ({
      title: c.web.title,
      uri: c.web.uri
    })) : [];
    res.json({
      ...finalResult,
      sources: finalResult.verified_profiles?.map((vp) => ({ title: `${vp.name} Profile (${vp.status})`, uri: vp.url })).concat(sources) || sources,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    });
  } catch (error) {
  ...
  After they reply to this, say: "Great. Thanks again for your time \u2014 I really enjoyed learning about your background. We'll be in touch about next steps soon. Take care." and wrap up.`;
    ...
    if (Array.isArray(history) && history.length > 0) {
      ...
      for (const h of history) {
        const role2 = h.role === "model" ? "model" : "user";
        contents.push({ role: role2, parts: [{ text: h.text || h.content || "" }] });
      }
    } else {
      contents.push({ role: "user", parts: [{ text: "Start the interview screening." }] });
    }
...
var ttsInstances = {};
async function getTtsInstance(voiceName) {
...
function parseNvidiaJson(raw) {
  const stripped = stripThinkBlock(raw);
  const cleaned = stripped.replace(/^```(?:json)?\s*/i, "").replace(/\s*```\s*$/i, "").trim();
...
app.post("/api/nvidia/summarize", async (req, res) => {
  ...
  try {
    ...
    const transcript = history.map((h) => `${h.role === "model" || h.role === "assistant" ? "Interviewer" : "Candidate"}: ${h.text || h.content || ""}`).join("\n\n");
...
var botStatusMap = /* @__PURE__ */ new Map();
var recordingsDir = path.join(process.cwd(), "public", "recordings");
...
app.patch("/api/meeting/app/bot/status", (req, res) => {
  ...
  console.log(`\u{1F916} Bot status update for candidate ${candidateId}:`, status);
  if (candidateId) {
...
app.patch("/api/meeting/app/bot/log", (req, res) => {
  ...
  console.log(`\u{1F916} Bot Log [${level.toUpperCase()}] for candidate ${botId || eventId}: ${message} (${category}/${subCategory})`);
  res.json({ success: true });
});
...
app.put("/api/files/upload/multipart/finalize/:teamId/:folderId/:fileId/:uploadId", async (req, res) => {
  ...
  const botId = fileMeta?.botId || fileId;
  ...
  try {
    ...
    const files = fs.readdirSync(chunkDir).filter((name) => name.startsWith("part_")).sort((a, b) => {
...
app.post("/api/meeting/webhook", (req, res) => {
  ...
  console.log("\u{1F916} Webhook received from meeting bot:", req.body);
...
app.post("/api/meeting/join", async (req, res) => {
  ...
  try {
    const response = await axios.post(`${botUrl}/${provider}/join`, {
      bearerToken: "local_meet_bot_key",
      // matching SCREENAPP_BACKEND_SERVICE_API_KEY
      url,
      name,
      teamId,
      timezone,
