...
def main():
    try:
        ...
        if 'import { LogOut }' not in content:
            content = "import { LogOut } from 'lucide-react';\n" + content
            
        layout_idx = content.find('function Layout(')
        if layout_idx == -1:
            print("Layout function not found")
            return
            
        return_idx = content.find('  return (', layout_idx)
        if return_idx == -1:
            print("Return not found")
            return
            
        pages_idx = content.find('// --- Pages ---', layout_idx)
        if pages_idx == -1:
